...
The government side of the model: adds a lump-sum tax and three product
taxes on top of the ConsumerClass from Consumer.py. set_taxes() translates
the taxes into the prices and income the consumer actually faces, so every
solution method inherited from ConsumerClass (.solve(), .solve_grid(), ...)
keeps working unchanged once taxes are set -- the consumer just optimizes
against the new, tax-inclusive prices/income.
"""
...
class GovernmentClass(ConsumerClass):
    ...
    def max_revenue(self,goods=(2,),tau_max=10.0,N=1001):
        ...
        for i,tau_i in enumerate(tau_grid):
            revenue[i],_ = self.revenue_and_utility(tau_i,goods)
    ...
    ############################################
    # 4. extension: revenue-neutral green reform
    ############################################

    def green_reform(self,tau_bus,subsidy_max=0.90):
        """Tax bus trips and use all revenue to subsidize train trips.

        The train subsidy is chosen so total product-tax revenue is zero.
        A subsidy is represented by a negative tax rate on train tickets.

        Returns:
            (SimpleNamespace): tax, subsidy, utility, quantities and emissions
        """

        # a. find the train subsidy that balances the budget
        def budget_balance(subsidy):
            self.set_taxes(tau2=tau_bus,tau3=-subsidy)
            opt = self.solve(do_print=False)
            return self.tax_revenue(opt)

        try:
            res = optimize.root_scalar(
                budget_balance,
                bracket=(0.0,subsidy_max),
                method='brentq'
            )
            subsidy = res.root
        except ValueError:
        ...
        if np.isnan(subsidy):
            ...
            out.x2 = np.nan
            out.x3 = np.nan
            out.emissions = np.nan
            return out

        # b. solve the consumer problem under the balanced reform
        self.set_taxes(tau2=tau_bus,tau3=-subsidy)
        opt = self.solve(do_print=False)
        x1,x2,x3 = self.quantities(opt.s1,opt.w)

        # c. a simple emissions index: bus emits more than train
        out.u = opt.u
        out.x1 = x1
        out.x2 = x2
        out.x3 = x3
        out.emissions = 1.0*x2 + 0.30*x3
